...
include ("connection.php");
   ...
   $email=$_SESSION['id'];
 list($username,$sessionid)=mysql_fetch_array(mysql_query("SELECT `fullname`,`email` FROM `admin` WHERE `email`='$email'  " ));
...
$image=$_FILES['image']['name'];
  ...
  mysql_query($sql);
	$image_check=$_FILES['image']['name'];
	 if($image_check!="")
	 {
		 $sql=mysql_query("UPDATE `admin` SET `image`='$image' WHERE `email`='$sessionid' ");
	 }
